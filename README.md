...
| エラー / 症状 | 原因と対処 |
|---|---|
| `'shx' は認識されていません` / `'cp' は認識されていません` | ビルドに使うツールが未インストール。プロジェクトのフォルダで `npm install` を実行し直す(shx は依存に含まれているので、これだけで入ります) |
| `'npm' は認識されていません` | Node.js が未インストール、またはインストール後に PowerShell を開き直していない。Node.js を入れて PowerShell を再起動 |
| `このシステムではスクリプトの実行が無効になっているため…` と出て npm が動かない | PowerShell の実行ポリシーが原因。`Set-ExecutionPolicy RemoteSigned -Scope CurrentUser` を実行して `Y` で許可し、もう一度試す |
| `.env が見つかりません` と表示される | `.env` の置き場所が違うか、`.env.txt` になっている。「4. `.env` を作成」のコマンドで作り直す |
| 起動するがコメントが流れてこない | 送信ページの `?room=` と `.env` の `ROOM_ID` が一致しているか確認 |
